...
const HiddenModelsModal = ({
    ...
    const [hiddenModels, setHiddenModels] = useState<CompanyModel[]>([]);
    ...
    const fetchHiddenModels = async () => {
        setLoading(true);
        try {
            const response = await axiosInstance.get(`/api/model/${companyId}/models/hidden`);
            if (response.data.success) {
                setHiddenModels(response.data.data);
            }
        } catch (error: any) {
            toast.error(error.response?.data?.message || "Failed to fetch hidden models");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleShowModel = async (modelId: string, modelName: string) => {
        const confirmed = window.confirm(`Are you sure you want to show ${modelName}?`);
        if (!confirmed) return;

        setIsToggling(true);
        const success = await hideModel(companyId, modelId);
        if (success) {
            setHiddenModels((prev) => prev.filter((m) => m._id !== modelId));
        }
        setIsToggling(false);
    };
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            Models that have been removed from the main view
                        </p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {loading ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-500">No hidden models found.</p>
                        </div>
                    ) : (
...
export default HiddenModelsModal;